...
def fit_legendre(x, y, yerr):               # Find the polynomial fit parameters and plot them
    inits = [1, 0, 0, 0, 0]
    params, covar = curve_fit(legendre, np.cos(x), y, sigma=yerr, p0=inits)     # Legendre actually is cos(x)
    domain = np.linspace(0, 140, 200)                                           # Range for plotting
    plt.plot(domain, legendre(np.cos(domain), *params), 'r--',                  # Plot fit over range
